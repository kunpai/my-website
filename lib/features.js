 ...
 * Still honours the older keys `features.workExperience` and top-level `enableChatbot`.
 */
function resolveFeatures(config = {}) {
    ...
    if (typeof raw.chatbot !== 'boolean' && typeof config.enableChatbot === 'boolean') {
        features.chatbot = config.enableChatbot;
    }
    return features;
}
...
function legacyFeatureWarnings(config = {}) {
    ...
    if ('enableChatbot' in config) warnings.push('enableChatbot is deprecated; use features.chatbot');
    return warnings;
}
